...
public class BST<T extends Comparable<? super T>> {
    ...
    public String getPath(T t1) {
		return this.root.el +" "+ getPath(t1,root);
		
	}
	private String getPath(T t1, BSTNode<T> root) {
		if(root.el.equals(t1)) {
			return "";}
		else if (t1.compareTo(root.el) < 0) {
			return root.left.el + " "+ getPath( t1, root.left);}
       else {
			return root.right.el +" "+  getPath( t1, root.right);}

	}
	
	public int getRightLeafCount() {
		return getRightLeafCount( root,null);
		
	}
	public int getRightLeafCount(BSTNode<T> root,BSTNode<T> previous) {
		if (previous == null)
			previous=root;
        if (root == null) 
            return 0; 
        if (root.left == null && root.right == null && previous.left!=root) 
            ...
            return getRightLeafCount(root.left,root) + getRightLeafCount(root.right,root);
	...
	public int rangeCounter(T t1, T t2,BSTNode<T> root) {
		if (root == null) {
            return 0; 
		} if((root.el.compareTo(t2) < 0) && (root.el.compareTo(t1) > 0)) {
	        return 1+rangeCounter( t1,  t2,root.left) + rangeCounter( t1,  t2,root.right);
		}
		return rangeCounter( t1,  t2,root.left) + rangeCounter( t1,  t2,root.right);
	...
	public T successor(T t1) {
		Stack<BSTNode<T>> travStack = new Stack<BSTNode<T>>();
		return successor(root, travStack, t1);

	}

	private T successor(BSTNode<T> root, Stack<BSTNode<T>> travStack, T t1) {
		if (root.el.equals(t1)) {
			if (root.right != null) {
				if (root.left == null)
			...
			} else if (root.right == null && travStack.topEl().left.el.equals(root.el)) {

				return travStack.pop().el;
			} else if (root.right == null && travStack.topEl().right.el.equals(root.el)) {
				while (t1.compareTo(travStack.topEl().el) > 0) {
					travStack.pop();
				}
				return travStack.pop().el;
			}
		...
		if (t1.compareTo(root.el) < 0) {
			travStack.push(root);
			return successor(root.left, travStack, t1);
		} else {
			travStack.push(root);
			return successor(root.right, travStack, t1);
		}
	...
	public T predecessor(T t1) {
		Stack<BSTNode<T>> travStack = new Stack<BSTNode<T>>();
		return predecessor(root, travStack, t1);
	}

	public T predecessor(BSTNode<T> root, Stack<BSTNode<T>> travStack, T t1) {
		if (root.el.equals(t1)) {
			if (root.left != null) {
				if (root.right == null)
				return root.left.el;
			...
			}else if (root.left == null && travStack.topEl().right.el.equals(root.el)) {
				return travStack.pop().el;
			} else if (root.left == null && travStack.topEl().left.el.equals(root.el)) {
				while (t1.compareTo(travStack.topEl().el) < 0) {
					travStack.pop();
				}
				return travStack.pop().el;
		...
		if (t1.compareTo(root.el) < 0) {
			travStack.push(root);
			return predecessor(root.left, travStack, t1);
		} else {
			travStack.push(root);
			return predecessor(root.right, travStack, t1);
		}
	}
}